...
public class RandomStrikeOrDefendDrawPileToHandAction extends AbstractGameAction {
    private AbstractPlayer p;

    public RandomStrikeOrDefendDrawPileToHandAction(int amount) {
    ...
    public void update() {
        if (this.duration == Settings.ACTION_DUR_MED) {// 26
            ...
            CardGroup tmp = new CardGroup(CardGroupType.UNSPECIFIED);// 33
            Iterator var2 = this.p.drawPile.group.iterator();// 34

            AbstractCard card;
            while (var2.hasNext()) {
                card = (AbstractCard) var2.next();
                if (card.rarity == AbstractCard.CardRarity.BASIC) {// 35
